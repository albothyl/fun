	...
	$().ready(function()
	{
		...
		$("#btn_seWrite").click(function() {
			if(nullCheck("제목", $("#boardWriteTitle").val()) && nullCheck("본문", $("#ir1").val())) {
		    	...
		    	writeForm.action = "/board/SHTBoard/update.do";
		    	writeForm.method = "post";
		    	writeForm.subjectCode.value = <c:out value="${pageVO.pageSubjectCode}"/>; //게시글 번호
		    	writeForm.tradeNo.value     = <c:out value="${shtBoard.tradeNo}"/>;       //게시글 번호
		    	writeForm.email.value       = "<c:out value="${shtBoard.email}"/>";       //글쓴이
		    	writeForm.submit();
			}else {
				return false;
			};
			...
			<div id="main">
				...
				<form:form modelAttribute="pageVO" id="writeForm">		
					<table id="boardWriteTable">
						...
						<tr>
							<td><input type="text" maxlength="50" id="boardWriteTitle" name="title" value="${shtBoard.title}"/></td>
						</tr>
						<tr>
							<td height="10"></td>
						</tr>
						<tr>
							<td>
								<textarea name="content" id="ir1" rows="20">${shtBoard.content}</textarea>
							</td>
						</tr>
						<tr>
							<td height="5"></td>
						...
						<tr>
							<td align="right">
								<input type="button" id="btn_seWrite"class="button" value="글수정"/>
